 ...
 * Structural integrity only; candidate content is allowed and this never
...
const PLACEHOLDER_TOKENS = new Set(['placeholder', 'todo', 'tbd', 'foo', 'bar']);

function readJson(path) { return JSON.parse(readFileSync(path, 'utf8')); }
function isNFC(str) { return typeof str === 'string' && str === str.normalize('NFC'); }

/**
 * Detect authoring placeholders without ASCII word-boundary bugs.
 * JS `\b` is ASCII-oriented: `/\btodo\b/i` incorrectly matched the `todo`
 * inside Spanish/Portuguese `método` because the accented `é` was treated as
 * a non-word character. Tokenize Unicode letters/marks/numbers and compare
 * whole tokens instead.
 */
function hasPlaceholderText(text) {
  if (typeof text !== 'string') return false;
  if (PLACEHOLDER_PHRASES.some((pattern) => pattern.test(text))) return true;
  const tokens = text.toLocaleLowerCase('und').match(/[\p{L}\p{M}\p{N}_-]+/gu) ?? [];
...
function main() {
  ...
  const schemaCheck = (validator, data, file) => {
    totalFiles += 1;
    if (!validator(data)) fail(`Schema error in ${file}: ${JSON.stringify(validator.errors)}`);
  };
  const register = (id, type, file) => {
    if (!id) { fail(`Missing ${type} ID in ${file}`); return; }
    if (ids.has(id)) {
  ...
  if (!existsSync(conceptPath)) fail('Missing concepts/graph.json');
  else {
  ...
  for (const lang of listDirs(namesRoot)) {
    for (const filename of listJson(join(namesRoot, lang))) {
      ...
      for (const n of data.names ?? []) {
        ...
        for (const spelling of n.spellings ?? []) if (!isNFC(spelling.text)) fail(`${rel}: spelling '${spelling.text}' is not NFC`);
  ...
  function scanLexicon(data, rel) {
    ...
    for (const lex of data.lexemes ?? []) {
      ...
      for (const sense of lex.senses ?? []) {
        ...
        for (const [ilang, definition] of Object.entries(sense.definitions ?? {})) {
          if (hasPlaceholderText(definition)) fail(`${rel}:${lex.lemma_nfc}: placeholder definition [${ilang}] '${definition}'`);
        }
        for (const field of ['homophones','homonyms','synonyms','antonyms','confusable_senses']) {
          for (const relation of sense[field] ?? []) pendingRelations.push({ relation, file: rel, owner: sense.sense_id, field });
        }
      }
  ...
  for (const lang of listDirs(languagesRoot)) {
    ...
    if (existsSync(profilePath)) schemaCheck(validateProfile, readJson(profilePath), `languages/${lang}/profile.json`);
    else fail(`Missing profile.json in languages/${lang}`);
  ...
  for (const work of listDirs(worksRoot)) {
    ...
    for (const filename of listJson(lexDir)) scanLexicon(readJson(join(lexDir, filename)), `works/${work}/lexicon/${filename}`);
    ...
    for (const filename of listJson(entityDir)) {
      ...
      for (const e of data.entities ?? []) {
        ...
        for (const label of e.labels ?? []) for (const component of label.components ?? []) pendingLabelComponents.push({ component, file: rel, owner: e.entity_id });
      }
    }
  ...
  for (const { relation, file, owner, field } of pendingRelations) {
    ...
    if (!target) fail(`${file}:${owner}:${field}: target '${relation.target_id}' does not exist`);
    else if (target.type !== relation.target_type) fail(`${file}:${owner}:${field}: target '${relation.target_id}' declared ${relation.target_type} but is ${target.type}`);
  }
  ...
  for (const { component, file, owner } of pendingLabelComponents) {
    ...
    if (!target) fail(`${file}:${owner}: label component target '${component.target_id}' does not exist`);
    else if (target.type !== component.target_type) fail(`${file}:${owner}: label component '${component.surface}' expects ${component.target_type} but '${component.target_id}' is ${target.type}`);
  }

  for (const { annotation, file } of pendingAnnotations) {
    const target = ids.get(annotation.target.target_id);
    if (!target) fail(`${file}:${annotation.annotation_id}: target '${annotation.target.target_id}' does not exist`);
    else if (target.type !== annotation.target.target_type) fail(`${file}:${annotation.annotation_id}: target declared ${annotation.target.target_type} but is ${target.type}`);
    if (annotation.analysis_id) {
      const analysis = ids.get(annotation.analysis_id);
      if (!analysis || analysis.type !== 'analysis') fail(`${file}:${annotation.annotation_id}: analysis '${annotation.analysis_id}' does not exist`);
    }
  }
