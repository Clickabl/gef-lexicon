 ...
 *
 * This is the bridge between canonical lesson data and Lexi. It deliberately
 * does not manufacture dictionary definitions. Instead it packages:
 *   - language-neutral concepts;
 *   - attested/generated surface forms in each language;
 *   - localized lesson explanation copy;
 *   - surface/concept -> lesson links.
 *
 * Output:
...
function familyData() {
  ...
  const concepts = (manifest.concept_order ?? shards[0]?.doc?.concept_order ?? []).map((conceptKey) => ({
  ...
  for (const { path, doc } of shards) {
    const order = doc.concept_order ?? manifest.concept_order ?? [];
    for (const entry of doc.entries ?? []) {
      ...
      for (let index = 0; index < order.length; index += 1) {
        ...
        // The authored contract uses spaces around slash for alternatives.
        // A slash without surrounding spaces may be part of a compositional label.
        ...
        for (let variantIndex = 0; variantIndex < alternatives.length; variantIndex += 1) {
          const surface = alternatives[variantIndex];
          forms.push({
            concept_id: `FAMILY.${conceptKey}`,
...
function compileJson() {
  ...
  const surfaceLessonLinks = genderSurfaceLessonLinks();
  const lexicalRelations = spanishLexicalRelations();
...
function compileSqlite(document) {
  ...
  db.transaction(() => {
    ...
    for (const form of document.concept_forms) {
      insertForm.run(
        form.concept_id, form.language_tag, form.surface_nfc, form.normalized_lookup,
        form.form_role, form.review_state, form.trust_state, form.source_path,
      );
    }
...
console.log(`📚 Semantic lesson index: ${familyLanguages.size} family-vocabulary languages, ${document.concepts.length} family concepts, ${document.concept_forms.length} forms.`);
console.log(`🧭 Gender source copy: ${genderSourceLanguages.size} languages; surface lesson links: ${document.surface_lesson_links.length}.`);
